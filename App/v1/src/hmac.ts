// Vendored SHA-256 (FIPS 180-4) + HMAC-SHA256 (RFC 2104). Pure TS — RN's
// Hermes engine has no dependable WebCrypto, and paired-mode signing must
// not pull a native dependency. The byte-for-byte reference is the
// firmware's Firmware/v1/core/auth/auth_logic.c: the mirror verifies the
// HMAC this produces (control-protocol-spec §3 / firmware-security §5.4),
...
// so the device HMACs UTF-8 bytes — the app must sign the same bytes.
